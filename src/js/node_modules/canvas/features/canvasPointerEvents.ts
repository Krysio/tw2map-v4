...
export default function featureCanvasPointerEvents(
    ...
    element.addEventListener('mouseup', (e: MouseEvent) => {
        if (mouseState.move) {
            ...
            let [ x, y ] = state.getHoverTile();

            events.emit('click/tile', { x, y });
        }
    ...
    element.addEventListener("touchstart", (e: TouchEvent) => {
        ...
        if (e.touches.length > 1) {
            ...
            touchState.prev.a[0] = clientX;
            touchState.prev.a[1] = clientY;
    ...
    element.addEventListener("touchend", (e: TouchEvent) => {
        if (e.touches.length === 2) {
            ...
            if (touchState.move) {
                ...
                let [ prevX, prevY ] = touchState.prevTile,
                    [ currentX, currentY ] = state.getHoverTile();
                ...
                if (prevX !== null
                    ...
                    events.emit('click/tile', { x: prevX, y: prevY });
                }

                touchState.prevTile[0] = currentX;
                touchState.prevTile[1] = currentY;
            }
            touchState.touch = false;
            touchState.move = false;

            e.preventDefault(); // stop emulate mouse events
        }
    }, {
        capture: false,
        passive: false
