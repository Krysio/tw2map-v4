...
import {
    TILE_TYPE_BORDER, TILE_TYPE_BORDER_WATER,
    TILE_COUNT
} from "appConstants";

/******************************/

export default function commandLoadBackgroundData(
    events: Events,
    mapData: MapData
) {
    return {
        loadBackgroundData(
            ...
            for (let tilePos = 0; tilePos < TILE_COUNT; tilePos++) {
                ...
                if (result >>> 4) {
                    if ((result >> 2) & 1) {
                        mapData.tileSetType(tilePos, TILE_TYPE_BORDER);
                    } else {
                        mapData.tileSetType(tilePos, TILE_TYPE_BORDER_WATER);
                    }
                }
            }

            events.emit('changed data/background', null);
        }
    };
}