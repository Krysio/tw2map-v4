...
export default class MapData {
    ...
    tribeMap: TribeMap = {};
    world = new WorldDetails();
    downloadedDataTime: number = 0;
    tiles = new Uint8Array(MAP_SIZE * MAP_SIZE);

    constructor() {
        this.villageInterator = this.villageInterator.bind(this);
    }
    ...
    clearMapData(): void {
        ...
        this.characterMap = {};
        this.tribeMap = {};
        this.downloadedDataTime = 0;
        this.world = new WorldDetails();
    ...
    *villageInterator(): IterableIterator<Village> {
        for (let id in this.villageMap) {
            yield this.villageMap[ id ];
        }
    }

    *characterInterator(): IterableIterator<Character> {
        for (let id in this.characterMap) {
            yield this.characterMap[ id ];
        }
    }

    *tribeInterator(): IterableIterator<Tribe> {
        for (let id in this.tribeMap) {
            yield this.tribeMap[ id ];
        }
    }
    ...
    getItem = (
        ...
        switch (type) {
            case 'village': return this.getVillage(itemId);
            case 'character': return this.getCharacter(itemId);
            case 'tribe': return this.getTribe(itemId);
        }
        return null;
    }
    ...
    tileSetType(
        ...
        this.tiles[ slot ] = value;
    }

    tileGetType = (
        slot: number
    ): number => {
        return this.tiles[ slot ] || 0;
    }
}
