...
const debugLog = debug('app:libs:mm:hook');
...
export default class Hook {
    ...
    public bindComponent(
        ...
        debugLog('bindComponent', this.uuid, component);
    ...
    public viewComponentHasBeenRendered(): void {
        debugLog('viewComponentHasBeenRendered', this.uuid);

        let vC = this.viewComponent;

        vC.updateViewHasBeenRequest = false;

        if (this.state.hanged2Visible) {
            this.state.hanged2Visible = false;
            this.fixPosition();
        }
    ...
    show = (): void => {
        //debugLog('show', this.uuid, this.modalManager.isMoveing());
        ...
        if (this.state.hidden === true) {
            ...
            this.state.hanged2Visible = true;
    ...
    isMovering(): boolean {
        if (this.modalManager
            && this.modalManager.state.move.hook === this
        ) {
            return true;
        }
    ...
    willHaveInactived(): void {
        if (this.options.closeWhenInactive === true) {
            this.close();
        } else if (this.options.hideWhenInactive === true) {
            this.hide();
        }
    }
    ...
    setLayout(
        ...
        this.options.closeWhenInactive = layout.props.closeWhenInactive || false;
