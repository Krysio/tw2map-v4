...
const debugLog = debug('app:lib:ModalManager');
...
export default class ModalManager extends EventEmitter {
    ...
    protected onMouseUp = (
        ...
        // klikanie poza MM dezaktywuje wszystkie okienka
        if (move.move === false) {
            let flag = false;
            let node: HTMLElement = e.target as HTMLElement;

            while (node) {
                if (node === this.viewComponent.rootElement) {
                    flag = true;
                    break;
                }
            ...
            if (flag === false) for (let hookFromList of this.hookList) {
    ...
    protected onMouseMove = (
        ...
        if (move.move) {
            ...
            for (let i = 0; i < this.hookList.length; i++) {
                this.hookList[ i ].cursorHasBeenMoved();
            }
        }
    }
